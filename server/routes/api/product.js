...
cloudinary.config({
  cloud_name: 'mantri123',
  api_key: '926693233564914',
  api_secret: 'aUu5zmu7t9uD9dPIb4d7nHkeDmM',
});
  ...
  async (req, res) => {
    try {
      ...
      if (process.env.AWS_ACCESS_KEY_ID) {
        let { imageUrl, imageKey } = await s3Upload(image);
      }
      else {
        let upload = await uploadFromBuffer(image)
      }
