...
//adds RefreshTokenToHttpCookue
const addRefreshTokenToHttpCookie = (refreshToken, res) => {
  const date = new Date();
  const expiresAt = date.setDate(date.getDate() + 7);
...
router.post('/refresh-token', async (req, res) => {
  try {
    ...
    //   await generateAccessTokenUsingRefreshToken(
    //     refreshToken,
    //   );
    // addRefreshTokenToHttpCookie(newRefreshToken, res);
    // return res.status(201).send({ accessToken, newRefreshToken });
    return res.status(201).send({ refreshToken: 'testing' })
  } catch (e) {
    console.log(e)
    throw {};
  }
